...
class SyncController
{
    ...
    public function syncData()
    {
        ...
        header('X-Accel-Buffering: no');  // INDISPENSABLE POUR NGINX

        // 2. Éviter le verrouillage de session
        if (session_status() === PHP_SESSION_ACTIVE) {
            session_write_close();
        }

        // 3. Vider TOUS les buffers de sortie de PHP
        while (ob_get_level() > 0) {
            ob_end_clean();
        }
        ob_implicit_flush(true);

        // 4. Temps d'exécution illimité
        set_time_limit(0);
            ...
            foreach ($saisons as $saison) {
                foreach ($liste_epreuves as $epreuve) {
                    ...
                    foreach ($categories_genre as $cat_code => $cat_nom) {
                        ...
                        // Timeout court pour éviter de bloquer indéfiniment si l'API FFESSM rame
    ...
    private function sendSSE($progress, $message, $is_done = false, $is_error = false)
    {
        ...
        // --- SÉCURITÉ ANTI-CACHE : Envoie 4ko de vide pour forcer les serveurs récalcitrants ---
        echo str_pad('', 4096) . "\n";
